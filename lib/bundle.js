...
class Game {
  ...
  load() {
    this.catRate = 0.01;
    this.catRateIncrease = this.catRate/(this.fps*90);

    const bg = new createjs.Shape();
    bg.graphics.beginBitmapFill(bgImg).drawRect(0, 0, this.canvas.width, this.canvas.height - KITMO.HEIGHT);
    const carpet = new createjs.Shape();
    carpet.graphics.beginBitmapFill(carpetImg)
      .drawRect(0, 0, CARPET.WIDTH, this.canvas.height - KITMO.HEIGHT);

    const kitmo = new createjs.Container();
    kitmo.x = 0;
    kitmo.y = this.canvas.height - KITMO.HEIGHT;
    const kitmoBG = new createjs.Shape();
    kitmoBG.graphics.beginBitmapFill(kitmoImg).drawRect(0, 0, KITMO.WIDTH, KITMO.HEIGHT);
